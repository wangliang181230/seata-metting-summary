...
Meeting Host: 王良（[wangliang181230](https://github.com/wangliang181230)）

Attendees (PPMC only, <a href="#user-content-appendix-other-attendees">see Appendix for other attendees</a>):
@Jimin、@Jianbin、@Wangliang、@Yixia、@Liuyuecai

Next bi-weekly meeting:
- Time: 2024/2/17 is the Chinese Spring Festival holiday, skipped, tentatively scheduled for the 2024/3/2. (If there are any changes, we will notify separately. Please follow the email-list: dev@seata.apache.org, notifications@seata.apache.org)
- Host: （Redefine）
...
2. ASF ICLA contributor signature: @ Qingming
    - Public email template done
    - Contact the corresponding developer Done via email after database migration

3. Review email template: @ Qingming
    - Provide review email template writing
    - Feedback with issues

4. Incubation progress synchronization @ Qingming
    - The legal issue has been resolved, don
    - Reported to Apache Secretary Done

5. Supplementary documents: Development history done, user guide @ Xiangkun, configuration document split @ Jianbin, upgrade document split @ Jianbin, deployment document split @ Jianbin@ Slievrly @ Xiangkun @ Jianbin
...
7. Compliance and security@ Autumn Maple @ Qingming done
...
8. Baidu search traffic decline issue (according to Apache, not seata io)@ Autumn maple
...
9. Abandon the language sparrow; Migration of historical documents to wiki @ Yuecai @ Yixia @ Open Source Xia Tutor
    ...
    - Migration of documents related to summer code to the corresponding directory (priority given to project completion in this year's plan design)@ Jianbin @ Wangliang @ Yuecai done
...
10. Single test coverage target @ Jiawei @ Zhongxiang
    - Seata core Zhongxiang done
    - Seata common Jiawei done

11. Disposal of backlog PR @ Jianbin @ Monthly Finance
...
13. Branch maintenance. Maintenance plan public document@ Junmin done
    - Maintain by version, only maintain the last 3 versions (including the latest version) merge, done

14. Compatibility plan for new and old warehouses. Sort out APIs, @ Xiangkun (already sorted out, to be discussed separately)
    - User API sorting, Saga completed (12.9), done (12.23) @ Xiangkun done

15. Compatibility testing (JDK, Arch, Spring core+boot, Druid) @ Wang Liang
...
17. Baidu search traffic decline issue (according to Apache, not seata io)@ Autumn maple
    - Ranking has increased and the official website is at the top. Done (old domain name, completed)

18. Package migration: org.apache.seata
    - Merge the current PR as soon as possible and handle it uniformly@ Qingming done 2/3
...
1. Compliance and security@ Autumn Maple @ Qingming doing
    - Logo renovation@ Autumn maple done
    - English blog; From 0 to 1; Translate the existing one; Todo@ Yunqing has completed 16 articles, and all article title placeholders have been added. There are still 30-40 articles @ original new, and more than 10 have been completed in the past two weeks; (Expected to be completed in 1-1.5 months)
    - 41 front-end security vulnerabilities @ Qingming, awaiting review merge, (2/3: 18 remaining vulnerabilities remain unresolved)
    - Seata go has 5 remaining vulnerabilities @ yixia Monthly financial target next week (all fixed) done
    - There are still 125 seata samples left. @ Also to be launched in summer, with pre dependencies (PR has been submitted, create a new branch, merge it first, and run to see if there are any problems) @ Qingming to be confirmed
    ...
    - https://github.com/apache/incubator-seata/discussions/6233 Compliance related issues
        - External link: @ Qiufeng @ Qingming
    ...
    - Basic work completed:
        ...
        - Problem: The domain name of the official website document has not been switched. The introduction of slogan's seata in the first month has been improved in the origin section@ Qingming @ Qiufeng done
    - Part of page 301 redirects to 404 Not Found: @ Qiufeng @ Qingming
        - Overall solution: @ Qingming
            - Gateway:
            - Nginx:
        - PR:
            - https://seata.apache.org/zh-cn/docs/overview/what-is-seata.html (PR submitted) @ Qiufeng
    - When adding a node to a cluster, it is necessary to verify whether the node is legal: @ Jianbin (2/3 new)

2. Baidu search traffic decline issue (according to Apache, not seata io)@ Autumn maple
    ...
    - The description of the official website found on Baidu, content optimization, and current effects are shown in three screenshots: (priority processing)
        - ![image](https://github.com/apache/incubator-seata/assets/1527893/875e640e-fd40-4b25-be9a-993955bd8cd4)
        - ![image](https://github.com/apache/incubator-seata/assets/1527893/193e4b83-2673-43af-a7a9-68bd5637245a)
        - ![image](https://github.com/apache/incubator-seata/assets/1527893/e3ec5663-2e89-4fc3-9495-1a8108449605)
...
4. Compatibility plan for new and old warehouses. Sort out APIs, @ Xiangkun (already sorted out, to be discussed separately)
    - State Machine Designer Framework Selection and Basic Framework (12.9) First Version Completed (12.23) (PR Completed, awaiting review; after individual branch development is completed, submit to 2. x branch separately) @ Xiangkun

5. Log optimization: (Overall progress: less than 20%) (@ Qingming PR has been submitted and meets the requirements of internationalization. The stack generated has corresponding errorCode, and a link chain will be displayed in the error message to the FAQ page.)
    - Log formatting input: info, error (the format has been determined, replace the 'log. error' and other fields with the new todo);
    - Error code has been raised for PR, awaiting review @ Qingming (internal exceptions are all sea exceptions, packaged uniformly, please follow the specifications in the future)
    - Independent log printing (now framework and business logs mixed together) todo

6. Single test coverage target of 70% @ Zhongxiang leads @ Jiawei (2/3: no progress in the first two weeks)
    ...
    - Sort out the current module coverage document and lead the promotion of @ Jiawei @ Zhongxiang 1.8
    - What are the standards for writing unit tests, Junit? Mock, version, etc@ Zhongxiang
    - What is the ultimate goal of stock governance and what standards must be met for incremental growth?

7. Capacity building for integrated testing and regression @ Jiawei @ Xiong Jingliu
...
8. Compatibility testing (JDK, Arch, Spring core+boot, Druid) @ Wang Liang
...
10. Mock test @ Minghua (4 PR have been submitted, awaiting review merge)
    ...
    - Add test cases@ Minghua (PR has been merged, but single testing often fails and needs to be reviewed again to see if it can be put into use) (@ Qingming trace)

11. Disposal of backlog PR @ Jianbin @ Monthly Finance
    - Go: Some have already contacted the author (recently restarted review, the merger to be merged) @ Monthly Finance
    ...
    - Sort out which ones cannot be contacted, and directly handle them on your own. @ Monthly Finance before 1.27
    - Change the module name of go to Apache xxx @ Monthly Finance
...
3. Seata go backlog PR processing: There is still one PR review for 2023 that has not been completed yet: https://github.com/apache/incubator-seata-go/pull/600
   All other PR codes have been reviewed and 2 CI issues are still being addressed

12. Package migration: org.apache.seata
    - Merge the current PR as soon as possible and handle it uniformly@ Qingming done
...
14. Seata go direction
    - Compliance governance: @ Yixia @ Qingming @ Yuecai
        - Warehouse, governance of some dependency packages, CVE security vulnerabilities: done
        - ModelName to be adjusted
    - Construction and Implementation of the Seata Go Saga Framework @ Yixia @ Xiangkun @ Jingliu (Overall Progress: 60-70%)
    ...
    - Align the features and bugfix of Seata Java, currently aligned with version 1.5.1, which needs to be upgraded (modify according to 2. x and not maintain the dual branch strategy) @ Monthly Finance has already marked it
        - I have made a list: the list is clear, 2 issues have been submitted, and the remaining issues need to be submitted (one of which is being followed up on @ Ruling)

15. Authentication: @ Jiang Junmin
...
17. Used by reserved plan discussion @ Jianbin
...
18. API compatibility:
    - Priority compatibility for major scenarios: Completed, @ Yin Xiangkun done
    - Server compatibility:
        - Old io. seata class name exists in JSON serialization, completed @ Jianbin done
        - The data in Raft mode is compatible with each other and can only be upgraded to server2.1 or above (note: Raft currently has fewer users, so data compatibility does not need to be considered temporarily) @ Jianbin
    - RM/TM API: Share in groups and divide tasks. (@ Qingming trace)
        - Interceptor failure issue: Keep the name and package name of the old interceptor@ Qingming
        - TCC: @ Yixia
        - SAGA: @ Yin Xiangkun
        - RPC: @ Qingming
        - Universal: @ Wu Jiangke

---

# Matters to be discussed

1. `resource-id` + `application-name` unique identifier confirmation @ Jianbin has a lower priority and currently does not have time (we will initiate further discussions when we have time)
...
2. Seata go direction:
    - Ecological expansion: Bilibili and other service frameworks @ Qingming Byte @ Hong Fan. See if we can do it again next week or not by the end of January. (I talked to Byte's Cloud We GO and found that the article is hanging on both sides. They will help review it.)

3. Console token automatically refreshes @ Autumn Maple scheme completed done (pending voting, deadline Monday, 20231225) Junmin PR next week
...
4. `TYPE_SEATA_MERGE` messages require an independent thread pool to handle @Jianbin Todo
...
7. 2.1 version core synchronization: (marked with * as important, must be uploaded)
    ...
    - Console configuration (linked to various configuration centers for push pull), @ Wang Liang 2.1 version not available, separate meeting for discussion; (When the task is sent out, if the PR can be reused, reuse it; if not, restart the PR)
...
8. Multiple threaded branch registration rollback sequence error or omission
    ...
    - Resulting in issues with data dependent branches and incorrect rollback order@ Jianbin (Plan to be discussed: performance and consistency issues, need to be discussed in a meeting again)
...
9. Discussion on compatibility issues between high and low versions of Server:
    ...
    - Protocol solution:
        - Sofa Bolt: @ Please synchronize the issue with Yixia
        - Grpc:
        - Python:
